...
def create_sheet7(complete_data, metadata, output_dir, global_data, data):
    ...
    date_list2 = becgis.CommonDates([complete_data['etb'][1],
                                     complete_data['tr'][1],
                                     complete_data['recharge'][1]])
    date_list = becgis.ConvertDatetimeDate(date_list2, out='datetime')
...
#    pop_fh = becgis.MatchProjResNDV(lu_fh, pop_fh, pop_temp)
    ...
    rz_depth_tif = becgis.MatchProjResNDV(lu_fh, np.array([rz_depth_fh]), tf.mkdtemp())[0]
...
def create_sheet7_svg(basin, period, data, output, template=False):
    ...
    if not template:
        svg_template_path = r'C:\Anaconda2\Lib\site-packages\wa\Sheets\svg\sheet_7_month.svg'
    else:
  ...
  #  dict3 = {'-':'0', 'Incremental ET natural':'3', 'Landscape ET':'2', 'Non-consumptive':'1'}
    ...
    tempout_path = output.split('.')[:-1][0]+'.svg'
    tree.write(tempout_path)
    out_png = output

    subprocess.call([r'C:\Program Files\Inkscape\inkscape.exe', tempout_path,
