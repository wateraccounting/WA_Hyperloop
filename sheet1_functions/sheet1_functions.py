...
def create_sheet1(complete_data, metadata, output_dir, global_data):
    ...
    output_dir2 = os.path.join(output_dir, metadata['name'])
    if not os.path.exists(output_dir2):
        os.makedirs(output_dir2)
        
    output_fh_in, output_fh_out = create_sheet1_in_outflows(os.path.join(output_dir2,"sheet5_monthly"), metadata, output_dir2)
        
    outflow_values, outflow_dates = sum_ts(np.array([output_fh_out]))
    outflow_values *= metadata['qout_scale']
    
    if output_fh_in:
        inflow_values, inflow_dates = sum_ts(np.array([output_fh_in]))
        inflow_values *= metadata['qin_scale']
    ...
    for date in common_dates:
        ...
        results = calc_sheet1(entries, metadata['lu'], sheet1_lucs, metadata['recycling_ratio'], q_outflow, q_out_avg, output_dir2, q_in_sw = q_inflow)
        
        # Save the results of the current month.
        all_results.append(results)    
        
        # Create the csv-file.
        output_fh = os.path.join(output_dir2,'csvs_monthly','sheet1_{0}_{1}.csv'.format(date.year, str(date.month).zfill(2)))
        create_csv(results, output_fh)
    
        # Plot the actual sheet.
        wa.Sheets.create_sheet1(metadata['name'], '{0}-{1}'.format(date.year, str(date.month).zfill(2)), 'km3/month', output_fh, output_fh.replace('.csv','.pdf'))
    
    # Create some graphs.
    plot_storages(all_results, common_dates, metadata['name'], output_dir2)
    plot_parameter(all_results, common_dates, metadata['name'], output_dir2, 'utilizable_outflow')
    
    # Create yearly csv-files.
    yearly_csv_fhs = sh4.create_csv_yearly(os.path.split(output_fh)[0], os.path.join(output_dir2, "csvs_yearly"), year_position = [-11,-7], month_position = [-6,-4], header_rows = 1, header_columns = 3)
    
    # Plot yearly sheets.
    for csv_fh in yearly_csv_fhs:
        wa.Sheets.create_sheet1(metadata['name'], csv_fh[-8:-4], 'km3/year', csv_fh, csv_fh.replace('.csv','.pdf'))
...
def create_sheet1_in_outflows(sheet5_csv_folder, metadata, output_dir):
    ...
    #sh5_folder = os.path.join('D:\Cambodia\Sheet5',metadata['name']+'_OUTPUT','csvs_monthly')
...
def calc_basinmean(perc_fh, lu_fh):
    ...
    perc_fh = becgis.MatchProjResNDV(lu_fh, np.array([perc_fh]), output_folder)
    EWR = becgis.OpenAsArray(perc_fh[0], nan_values = True)
...
def create_csv(results, output_fh):
    ...
    writer.writerow(['INFLOW', 'PRECIPITATION', 'Rainfall', '{0:.2f}'.format(results['p_advection'])])
    writer.writerow(['INFLOW', 'PRECIPITATION', 'Snowfall', 0.])
    writer.writerow(['INFLOW', 'PRECIPITATION', 'Precipitation recycling', '{0:.2f}'.format(results['p_recycled'])])
    writer.writerow(['INFLOW', 'SURFACE WATER', 'Main riverstem', '{0:.2f}'.format(results['q_in_sw'])])
    ...
    writer.writerow(['INFLOW', 'GROUNDWATER', 'Natural', '{0:.2f}'.format(results['q_in_gw'])])
    writer.writerow(['INFLOW', 'GROUNDWATER', 'Utilized', 0.])
    writer.writerow(['INFLOW', 'OTHER', 'Desalinized', '{0:.2f}'.format(results['q_in_desal'])])
    writer.writerow(['STORAGE', 'CHANGE', 'Surface storage', '{0:.2f}'.format(results['dS'])])
    writer.writerow(['STORAGE', 'CHANGE', 'Storage in sinks', 0.])
    writer.writerow(['OUTFLOW', 'ET LANDSCAPE', 'Protected', '{0:.2f}'.format(results['landscape_et_plu'])])
    writer.writerow(['OUTFLOW', 'ET LANDSCAPE', 'Utilized', '{0:.2f}'.format(results['landscape_et_ulu'])])
    writer.writerow(['OUTFLOW', 'ET LANDSCAPE', 'Modified', '{0:.2f}'.format(results['landscape_et_mlu'])])
    writer.writerow(['OUTFLOW', 'ET LANDSCAPE', 'Managed', '{0:.2f}'.format(results['landscape_et_mwu'])])
    writer.writerow(['OUTFLOW', 'ET UTILIZED FLOW', 'Protected', '{0:.2f}'.format(results['uf_plu'])])
    writer.writerow(['OUTFLOW', 'ET UTILIZED FLOW', 'Utilized', '{0:.2f}'.format(results['uf_ulu'])])
    writer.writerow(['OUTFLOW', 'ET UTILIZED FLOW', 'Modified', '{0:.2f}'.format(results['uf_mlu'])])
    writer.writerow(['OUTFLOW', 'ET UTILIZED FLOW', 'Managed', '{0:.2f}'.format(results['uf_mwu'])])        
    writer.writerow(['OUTFLOW', 'ET INCREMENTAL', 'Manmade', '{0:.2f}'.format(results['manmade'])])  
    writer.writerow(['OUTFLOW', 'ET INCREMENTAL', 'Natural', '{0:.2f}'.format(results['natural'])])
    writer.writerow(['OUTFLOW', 'SURFACE WATER', 'Main riverstem', '{0:.2f}'.format(results['q_outflow'])])
    writer.writerow(['OUTFLOW', 'SURFACE WATER', 'Tributaries',  '{0:.2f}'.format(results['q_out_sw'])])  
    ...
    writer.writerow(['OUTFLOW', 'GROUNDWATER', 'Natural', '{0:.2f}'.format(results['q_out_gw'])]) 
    writer.writerow(['OUTFLOW', 'GROUNDWATER', 'Utilized', 0.])
    writer.writerow(['OUTFLOW', 'OTHER', 'Non-utilizable', '{0:.2f}'.format(results['non_utilizable_outflow'])])
    writer.writerow(['OUTFLOW', 'OTHER', 'Other', '{0:.2f}'.format(results['other'])])
    writer.writerow(['OUTFLOW', 'RESERVED', 'Commited', '{0:.2f}'.format(results['reserved_outflow_actual'])]) 
...
#    data : str
#        A csv file that contains the water data. The csv file has to
