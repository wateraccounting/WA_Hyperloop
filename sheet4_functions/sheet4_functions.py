...
def create_gw_supply(metadata, complete_data, output_dir):
    ...
    common_dates = becgis.common_dates([complete_data['supply_total'][1], complete_data['supply_sw'][1],
                                        complete_data['p'][1]])
...
def create_sheet4_6(complete_data, metadata, output_dir, global_data):
    ...
    if lu_based_supply_split:
        ...
        mask = (~np.isnan(becgis.open_as_array(metadata['lu'], nan_values=True))).astype(int)
        mask[mask==0] = -9999
        sw_supply_fraction_tif = os.path.join(output_dir2, 'sw_supply_fraction.tif')
        becgis.create_geotiff(sw_supply_fraction_tif, mask, driver, NDV, xsize, ysize, GeoT, Projection)
    ...
    for csv_file in csv6:
        year = csv_file[-8:-4]
        create_sheet6(metadata['name'], year, 'km3/year', csv_file, csv_file.replace('.csv', '.pdf'), template = get_path('sheet6_svg'), smart_unit = True)
    
    for cv in sheet4_csv_yearly:
        year = int(cv[-8:-4])
        create_sheet4(metadata['name'], '{0}'.format(year), ['km3/year', 'km3/year'], [cv, cv], 
                          [cv.replace('.csv','_a.pdf'), cv.replace('.csv','_b.pdf')], template = [get_path('sheet4_1_svg'), get_path('sheet4_2_svg')], smart_unit = True)
...
def update_irrigation_fractions(lu_tif, fraction_tif, lucs, equiped_sw_irrigation_tif):
    ...
    sw_tif = becgis.match_proj_res_ndv(lu_tif, np.array([equiped_sw_irrigation_tif]), tf.mkdtemp())[0]
    ...
    becgis.create_geotiff(fraction_tif, FRACTIONS, driver, NDV, xsize, ysize, GeoT, Projection)
...
def plot_per_category(fhs, dates, lu_fh, dictionary, output_fh, scale = 1e-6, gradient_steepness = 2, quantity_unit = ['ET', 'mm/month']):
    ...
    for key in list(ets_accumulated.keys()):
        if np.any([np.nansum(ets_accumulated[key]) <= 0.01 * np.nansum(list(ets_accumulated.values())), np.isnan(np.nansum(ets_accumulated[key]))]):
            continue
        else:
...
def split_flows(flow_fh, fraction_fh, output_folder, date, flow_names = ['sw','gw']):
    ...
    becgis.create_geotiff(flow_one_fh, FLOW_one, driver, NDV, xsize, ysize, GeoT, Projection)
    becgis.create_geotiff(flow_two_fh, FLOW_two, driver, NDV, xsize, ysize, GeoT, Projection)
...
def fractions(lu_fh, fractions, lucs, output_folder, filename = 'fractions.tif'):
    ...
    becgis.create_geotiff(fraction_fh, FRACTION, driver, NDV, xsize, ysize, GeoT, Projection)
    return fraction_fh
...
def create_sheet6_csv(entries, entries_2, lu_fh, lucs, date, output_folder, convert_unit = 1):
    ...
    for SUBTYPE in list(results_sh6.keys()):
        for TYPE in list(results_sh6[SUBTYPE].keys()):
...
# 
    ...
    common_dates = becgis.common_dates([complete_data['supply_total'][1], #complete_data['etb'][1], 
                                      complete_data['dro'][1], 
...
#            PERC[LULC == code] = PERC[LULC == code] - DPERC[LULC == code]
        ...
        becgis.create_geotiff(outfile_sup, SUP, driver, NDV, xsize, ysize, GeoT, Projection)
        
        outfile_dro = os.path.join(directory_dro, os.path.basename(dro_tif))
        becgis.create_geotiff(outfile_dro, DRO, driver, NDV, xsize, ysize, GeoT, Projection)
        
        outfile_sro = os.path.join(directory_sro, os.path.basename(sro_tif))
        becgis.create_geotiff(outfile_sro, SRO, driver, NDV, xsize, ysize, GeoT, Projection)

        outfile_dperc = os.path.join(directory_dperc, os.path.basename(dperc_tif))
        becgis.create_geotiff(outfile_dperc, DPERC, driver, NDV, xsize, ysize, GeoT, Projection)
...
#        becgis.create_geotiff(outfile_perc, PERC, driver, NDV, xsize, ysize, GeoT, Projection)
        ...
        becgis.create_geotiff(outfile_tr, TR, driver, NDV, xsize, ysize, GeoT, Projection)


    complete_data['supply_total'] = becgis.sort_files(directory_sup, [-10,-6], month_position = [-6,-4])[0:2]
    complete_data['dro'] = becgis.sort_files(directory_dro, [-10,-6], month_position = [-6,-4])[0:2]
    complete_data['sr'] = becgis.sort_files(directory_sro, [-10,-6], month_position = [-6,-4])[0:2]
    complete_data['dperc'] = becgis.sort_files(directory_dperc, [-10,-6], month_position = [-6,-4])[0:2]
#    complete_data['perc'] = becgis.sort_files(directory_perc, [-10,-6], month_position = [-6,-4])[0:2]
    complete_data['tr'] = becgis.sort_files(directory_tr, [-10,-6], month_position = [-6,-4])[0:2]
...
def bf_reduction_with_gwsup(metadata, complete_data):
    ...
    for date in common_dates:        
        ...
        becgis.create_geotiff(outfile_bf, BF_new, driver, NDV, xsize, ysize, GeoT, Projection)
        outfile_tr = os.path.join(directory_ro, os.path.basename(ro_tif))
        becgis.create_geotiff(outfile_tr, RO_new, driver, NDV, xsize, ysize, GeoT, Projection)
    complete_data['bf'] = becgis.sort_files(directory_bf, [-10,-6], month_position = [-6,-4])[0:2]
    complete_data['tr'] = becgis.sort_files(directory_ro, [-10,-6], month_position = [-6,-4])[0:2]
    return complete_data
